        ...
        : (ArnLightsourceCollector *) lightsourceCollector
{
    ...
    ArLightIntensity  * tempRadiance = arlightintensity_alloc( art_gv );
    ArLight           * tempLight = arlight_alloc( art_gv );
    ...
    for ( int i = 0; i < numberOfSunSamples; i++ )
    {
        ...
        for ( int a = 0; a < PSM_ARRAYSIZE; a++ )
        {
            ...
            if ( t <= 0. )
            {
                ...
                [ SKYDOME_ENVIRONMENT_MATERIAL lightEmittedTowardsPointFromDirection
                    :   sp
                    : & RAY3D_V(ray)
                ...
                arlight_l_init_i( art_gv, tempLight, tempRadiance );
    ...
    arlight_free( art_gv, tempLight );
    arlightintensity_free( art_gv, tempRadiance );
    ...
    for ( int a = 0; a < PSM_ARRAYSIZE; a++ )
    {
        ...
        patch[SUN_INDEX].radiantPower[a] =
            arlightintensity_i_norm_visible_range(
                art_gv,
                patch[SUN_INDEX].spectralPower[a]
                );
